...
function downloadAsPDF() {
    ...
    if (division) dataToExport = dataToExport.filter(s => s.division === division);
    if (teacher) dataToExport = dataToExport.filter(s => s.teacher_name.toLowerCase().includes(teacher));
    if (date) dataToExport = dataToExport.filter(s => s.date === date);
    if (dataToExport.length === 0) { alert("No data to export based on the current filters."); return; }
    ...
    doc.autoTable({ head: [tableColumn], body: tableRows, startY: 22, theme: 'grid', styles: { fontSize: 8 }, headStyles: { fillColor: [13, 71, 161] } });
