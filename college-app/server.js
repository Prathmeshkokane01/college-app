// THIS LINE IS VERY IMPORTANT AND MUST BE AT THE TOP
...
app.delete('/api/remove-fine', async (req, res) => {
    ...
    try {
        const result = await pool.query(
            `DELETE FROM "Fines" WHERE student_roll_no = $1 AND student_division = $2 AND date = $3 AND lecture_time = $4`,
            [rollNo, division, date, lectureTime]
        );
        if (result.rowCount > 0) {
            res.json({ success: true, message: `Fine removed successfully.` });
        } else {
            res.status(404).json({ success: false, message: 'No matching fine found to remove.' });
        }
    } catch (err) {
    ...
    try {
        ...
        if (absentRollNos && absentRollNos.length > 0) {
            ...
            for (const rollNo of absentRollNos) {
                await pool.query(sql, [rollNo, division, date, lectureTime, fineAmount, lectureName, topic]);
            }
        }
