...
const HOD_PASSWORD = "1234";
const teachers = [ { email: 'prathmeshkokane2511@gmail.com', code: '1230' }, { email: 'prof.sharma@college.edu', code: 'SHA_CS101' }, { email: 'prof.patel@college.edu', code: 'PAT_CS202' }, ];
...
app.post('/api/submit-attendance', async (req, res, next) => {
    try {
        ...
        next(err); // Pass the error to the new generic handler
    }
});

app.get('/api/students/:division', async (req, res, next) => {
    try {
...
app.get('/api/all-submissions', async (req, res, next) => {
    try {
...
// ** NEW: Generic Error Handler - This MUST be the last `app.use()` **
app.use((err, req, res, next) => {
    console.error('--- UNHANDLED ERROR ---');
    console.error(err.stack);
    res.status(500).json({ success: false, message: 'An unhandled server error occurred.' });
});
